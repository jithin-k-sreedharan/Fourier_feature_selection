...
def plot_ccm_comparison(xaxis, data_name, scores, file_name=None):
    ...
    Fourier_FS_length = min(
        len(scores["FL-Depth-2"]), len(scores["FL-Depth-1"]))
    for k, v in scores.items():
        plt.plot(xaxis[:Fourier_FS_length],
                 v[:Fourier_FS_length], label=k, alpha=0.7)
        plt.legend(loc='best')
        plt.xlabel(r"$k$")
...
def main(DATA_NAME):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=random_state_temp, stratify=y, shuffle=True)
    ...
    if not PARALLELIZE_CV and ORTHOGONALIZE:
        ...
        sel_features_UFFS_X = fourier_learning.UnsupervisedFourierFS(
            X, mean_emp_X, std_emp_X)
