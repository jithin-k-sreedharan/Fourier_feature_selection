...
def center(X):
    ...
    mean_col = tf.reduce_mean(input_tensor=X, axis=0, keepdims=True)
    mean_row = tf.reduce_mean(input_tensor=X, axis=1, keepdims=True)
    mean_all = tf.reduce_mean(input_tensor=X)
...
class CCM(object):
    ...
    def __init__(
        ...
        with tf.Graph().as_default():
            self.w = tf.compat.v1.placeholder(tf.float64, shape=d)
            self.inputs = [self.w]
            ...
            self.loss = tf.linalg.trace(tf.matmul(
                    Y, tf.matmul(G_X_w_inv, Y), transpose_a=True))

            self.gradients = tf.gradients(ys=self.loss, xs=self.inputs)
